	...
	__try
	{
		...
		while (TRUE)
		{
			if (MmIsAddressValid(pCallbackEntry->CallbackEntry->Altitude.Buffer))
			{
				...
				kprintf("%s: PreOperation Callback at:%p, Found PostOperation Callback at:%p\n", __FUNCTION__,  (UINT64*)pCallbackEntry->PreOperation, (UINT64*)pCallbackEntry->PostOperation);
